...
def extract_full_pdf_text(pdf_path: str, page_break_marker: str = "\n------page break------\n") -> str:
    ...
    if len(text) < (AVERAGE_NUM_CHARS_PER_PAGE_EXPECTATION * len(doc)):
        logging.warning(f"PDF {pdf_path} averages less than {AVERAGE_NUM_CHARS_PER_PAGE_EXPECTATION} characters per page, this PDF may require OCR processing.")
